...
在最后**必须手动模拟整个操作流程**（通过api调用、浏览器模拟等方式），确保完整符合需求的所有要求，否则就得改到测试通过为止。对于需求中没有明确的一些基本的UI/UX问题也要站在用户角度考虑测到位（例如刷新时窗口抖动等问题）

除非测试环境出现无法解决的问题（如中转api挂了）可以暂停测试向用户报告。

测试完毕后关掉测试用的所有监听服务。

## 测试额外要求

1. 要求测较长的流式输出能否完整同步、正确显示的问题
