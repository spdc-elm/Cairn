...
from cairn.server.routers.executions import create_project_execution, dispatcher_append_execution_events, dispatcher_lease_pending_execution
...
class V34EventIdempotencyTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        pending = create_project_execution(self.project_id, CreateExecutionRequest(task_type="explore", phase="bootstrap"))
        self.execution = dispatcher_lease_pending_execution(
            LeaseExecutionRequest(execution_id=pending.id, dispatcher_id="disp", worker_name="mock", worker_type="mock")
        )
    ...
    def test_same_event_key_same_canonical_event_replays_existing_row(self) -> None:
        body = AppendExecutionEventsRequest(
            dispatcher_id="disp",
            sink_token=self.execution.sink_token,
            events=[
